...
*/
static int32 OnAudioCapCallback(void *OutBuffer, void* InBuffer, uint32 InBufferFrames, double StreamTime, RtAudioStreamStatus AudioStreamStatus, void* InUserData)
{
	...
	FAudioRecManager* AudioRecManager = (FAudioRecManager*)InUserData;

	// Call the mic capture callback function
	return AudioRecManager->OnAudioCapture(InBuffer, InBufferFrames, StreamTime, AudioStreamStatus == RTAUDIO_INPUT_OVERFLOW);
}

/**
* FAudioRecManager Implementation
*/

FAudioRecManager::FAudioRecManager()
...
FAudioRecManager& FAudioRecManager::Get()
{
	// Static isngleton instance
	static FAudioRecManager MicInputManager;
	return MicInputManager;
}

void FAudioRecManager::StartRecording(const FRecSettings& InSettings, TArray<USoundWave*>& OutSoundWaves, int32 SampleRate)
{
	WAVE_FILE_SAMPLERATE=SampleRate;
	if (bError)
	{
		return;
	}
	...
	try
	{
		...
		ADC.openStream(nullptr, &StreamParams, RTAUDIO_SINT16, RecordingSampleRate, &BufferFrames, &OnAudioCapCallback, this);
	}
	catch (RtAudioError& e)
	{
		FString ErrorMessage = FString(e.what());
...
static void ConvertSampleRate(float CurrentSR, float TargetSR, int32 NumChannels, const TArray<int16>& CurrentRecordedPCMData, int32 NumSamplesToConvert, TArray<int16>& OutConverted)
{
...
void FAudioRecManager::StopRecording(TArray<USoundWave*>& OutSoundWaves)
{
	...
	if (bRecording)
	{
		...
		if (CurrentRecordedPCMData.Num() > 0)
		{
			...
			{
				UE_LOG(LogMicManager, Log, TEXT("Converting sample rate from %d hz to %d hz."), (int32)RecordingSampleRate, WAVE_FILE_SAMPLERATE);

				ConvertSampleRate(RecordingSampleRate, (float)WAVE_FILE_SAMPLERATE, NumInputChannels, CurrentRecordedPCMData, NumRecordedSamples, ConvertedPCMData);
...
int32 FAudioRecManager::OnAudioCapture(void* InBuffer, uint32 InBufferFrames, double StreamTime, bool bOverflow)
{
	FScopeLock Lock(&CriticalSection);
