...
void USpeechRecognition::SetApiKey(FString ApiKey)
{
    APIKEY = conversions::to_string_t(TCHAR_TO_UTF8(*ApiKey));
}

USpeechRecognition* USpeechRecognition::GoogleSTT(FGoogleSTTConfig config)
{
    USpeechRecognition* BlueprintNode = NewObject<USpeechRecognition>();
...
void get_results(string_t STTaudio, Config STTconfig, FString& Result, USpeechRecognition* BlueprintNode)
{
...
void USpeechRecognition::Recognize(FGoogleSTTConfig GoogleSTTConfig, USpeechRecognition* BlueprintNode)
{
    check(GoogleSTTConfig.SoundWave->RawPCMData != nullptr);
